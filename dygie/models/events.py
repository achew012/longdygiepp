...
class EventExtractor(Model):
    ...
    def __init__(self,
        ...
        trigger_class_weights = {name: vocab.get_token_to_index_vocabulary(name)
                                  for name in self._trigger_namespaces}
        arg_class_weights = {name: vocab.get_vocab_size(name)
                                   for name in self._argument_namespaces}
 
        print('DEBUGGG', self._n_trigger_labels)
        print('DEBUGGG', self._n_argument_labels)

        # Trigger and argument loss.
        #self._trigger_loss = torch.nn.CrossEntropyLoss(weight=trigger_class_weights, reduction="sum")
        #self._argument_loss = torch.nn.CrossEntropyLoss(weight=arg_class_weights, reduction="sum", ignore_index=-1)
